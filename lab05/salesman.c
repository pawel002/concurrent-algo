...
int main(int argc, char** argv) 
{
    ...
    const int N = 14;
    const int s = 1; // set to 1 to save graph x, y and solution into file
    ...
    for(int i = 0; i < g.n; i++)
    {
        ...
        // g.x[i] = (float)(rand() % 100);
        // g.y[i] = (float)(rand() % 100);
    }

    // randomy shuffle nodes of G
    ...
    if (rank == 0) master(size - 1, &g, task_type, result_type, s);
